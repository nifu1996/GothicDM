function FAI_KEILER_ATTACK(player)
    ...
    if(GetDistancePlayers(player.ID, player.ENEMY[1]) > AttackRange)then
        if(player.Aivars["RUNMODE"] ~= nil and player.Aivars["RUNMODE"] == 0)then
            ...
            table.insert(player.NEXTMOVES, {type=2, waittime=300}); -- 200, bestimmt die Zeitspanne zwischen den Angriffen
...
function ON_KEILER_HIT(player, targetid)
    ...
    if(math.random() > 0.4)then
        table.insert(player.NEXTMOVES, {type=1, anim="T_FISTPARADEJUMPB"});
        table.insert(player.NEXTMOVES, {type=2, waittime=math.random(0,400)}); --0/400 --waittime=math.random(500,1000)})
