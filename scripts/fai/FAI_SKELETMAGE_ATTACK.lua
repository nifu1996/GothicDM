function FAI_SKELETMAGE_ATTACK(player)
    ...
    if(GetDistancePlayers(player.ID, player.ENEMY[1]) > AttackRange)then -- Distanz zwischen Monster und Spieler > Attack Range....
        if(player.Aivars["RUNMODE"] ~= nil and player.Aivars["RUNMODE"] == 0)then
            ...
            elseif(player.WeaponMode == 4) then
                ...
                table.insert(player.NEXTMOVES, {type=1, anim="t_MagRun_2_FBTShoot"});
            end
        else
            --table.insert(player.NEXTMOVES, {type=1, anim="S_FISTRUNL"});
            if(player.WeaponMode == 3) then
            ...
            elseif(player.WeaponMode == 4) then
                ...
                table.insert(player.NEXTMOVES, {type=1, anim="s_FBTShoot"});
            end
        end;
		...
		if(dangle > -12 and dangle < 12)then
			...
			elseif(player.WeaponMode == 4) then
				...
				table.insert(player.NEXTMOVES, {type=1, anim="t_FBTShoot_2_Stand"});
			end
			table.insert(player.NEXTMOVES, {type=3, victim=player.ENEMY[1]}); -- Monster hat gehittet
			table.insert(player.NEXTMOVES, {type=2, waittime=math.random(500,1000)}); -- 200, bestimmt die Zeitspanne zwischen den Angriffen
			...
			table.insert(player.NEXTMOVES, {type=2, waittime=math.random(500,1000)}); --0/400 --waittime=math.random(500,1000)})
		end
    end
...
function ON_SKELETMAGE_HIT(player, targetid)
