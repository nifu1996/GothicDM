function Swamprat()
	local npc = CreateNPC(GetNewNPCName("Sumpfratte"));
	
	if(npc == -1)then
		print("Creating Swamprat Failed!");
	end
	
	SetPlayerInstance(npc,"Swamprat");
	
	SetPlayerStrength(npc, 55);
    SetPlayerDexterity(npc, 55);
    SetPlayerLevel(npc, 12);
    SetPlayerMaxHealth(npc, 390);
    SetPlayerHealth(npc, 390);
	
	npcarr = {};
	npcarr.id = npc;
	npcarr.daily_routine = nil;
	npcarr.attack_routine = FAI_WOLF_ATTACK;
	npcarr.WeaponMode = 0;
	npcarr.dialogs = nil;
	npcarr.update_func = AI_UP_MONSTER;
	npcarr.target_routine = AI_TA_MONSTER;
	npcarr.onhitted = ON_WOLF_HIT;
	npcarr.Guild = AI_GUILD_Swamprat;
	npcarr.func = Swamprat;
	npcarr.instance = "Swamprat";
	npcarr.respawntime = 1500;
	return npcarr;
end

function OW_Swamprat()
	local npc = CreateNPC(GetNewNPCName("Sumpfratte"));
	
	if(npc == -1)then
		print("Creating OW_Swamprat Failed!");
	end
	
	SetPlayerInstance(npc,"Swamprat");
	
	SetPlayerStrength(npc, 55);
    SetPlayerDexterity(npc, 55);
    SetPlayerLevel(npc, 12);
    SetPlayerMaxHealth(npc, 390);
    SetPlayerHealth(npc, 390);
	
	npcarr = {};
	npcarr.id = npc;
	npcarr.daily_routine = nil;
	npcarr.attack_routine = FAI_WOLF_ATTACK;
	npcarr.WeaponMode = 0;
	npcarr.dialogs = nil;
	npcarr.update_func = AI_UP_MONSTER;
	npcarr.target_routine = AI_TA_MONSTER;
	npcarr.onhitted = ON_WOLF_HIT;
	npcarr.Guild = AI_GUILD_Swamprat;
	npcarr.func = Swamprat;
	npcarr.instance = "OW_Swamprat";
	npcarr.respawntime = 1500;
	return npcarr;
end

function Spawn_Swamprat()
	local npc = CreateNPC(GetNewNPCName("Sumpfratte"));
	
	if(npc == -1)then
		print("Creating SpawnSwamprat Failed!");
	end
	
	SetPlayerInstance(npc,"Swamprat");
	
	SetPlayerStrength(npc, 55);
    SetPlayerDexterity(npc, 55);
    SetPlayerLevel(npc, 12);
    SetPlayerMaxHealth(npc, 390);
    SetPlayerHealth(npc, 390);
	
	npcarr = {};
	npcarr.id = npc;
	npcarr.daily_routine = nil;
	npcarr.attack_routine = FAI_WOLF_ATTACK;
	npcarr.WeaponMode = 0;
	npcarr.dialogs = nil;
	npcarr.update_func = AI_UP_MONSTER;
	npcarr.target_routine = AI_TA_MONSTER;
	npcarr.onhitted = ON_WOLF_HIT;
	npcarr.Guild = AI_GUILD_Swamprat;
	npcarr.func = Swamprat;
	npcarr.instance = "Human";
	npcarr.respawntime = 2147483;
	return npcarr;
end