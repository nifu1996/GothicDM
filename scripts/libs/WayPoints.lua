...
function FreePointList:load(file)
    if WPConverter == true then  
        ...
        for line in fh:lines() do --reading all lines. Each Line = 1 WP
            --local values = string.split(line,";"); -- Bei Windows Version nutzen
            local values = string.split(line:gsub('\r', ''),";"); -- Bei Linux Version nutzen
            ...
            --InsWaypoint = mysql_query(mysql_handler, "INSERT INTO `Freepoints` (`Name`, `x`, `y`, `z`, `dirx`, `dirz`, `World`) VALUES ('"..wp.name.."', '"..wp.x.."','"..wp.y.."','"..wp.z.."','"..wp.dirx.."','"..wp.dirz.."','"..mysql_escape_string(mysql_handler,"NEWWORLD\\KHORINISDM.ZEN").."')")
            --(wp.name,wp.x,wp.y,wp.z,wp.dirx,wp.dirz,"NEWWORLD\\KHORINISDM.ZEN")

            InsWaypoint = mysql_query(mysql_handler, "INSERT INTO `Waypoints` (`Name`, `x`, `y`, `z`, `World`) VALUES ('"..wp.name.."', '"..wp.x.."','"..wp.y.."','"..wp.z.."','"..mysql_escape_string(mysql_handler,"NEWWORLD\\KOLONIEDM.ZEN").."')")
...
function WayNet:load(file)
    if WPConverter == true then  
        ...
        for line in fh:lines() do --reading all lines. Each Line = 1 WP
            ...
            wp.dirx = tonumber(values[5]);
            wp.dirz = tonumber(values[6]);
            InsWaypoint = mysql_query(mysql_handler, "INSERT INTO `Waypoints` (`Name`, `x`, `y`, `z`, `dirx`, `dirz`, `World`) VALUES ('"..wp.name.."', '"..wp.x.."','"..wp.y.."','"..wp.z.."','"..wp.dirx.."','"..wp.dirz.."','"..mysql_escape_string(mysql_handler,"NEWWORLD\\KOLONIEDM.ZEN").."')")
            --Remove the first 4 values from 4 to 1. After that the table includes only the next waypoints
            table.remove(values,6);
            table.remove(values,5);
            table.remove(values,4);
            ...
            for key, otherwp in pairs(values) do  --Exporter von Textdatei zu Datenbank
                print(wp.name,"Verbunden mit :",otherwp);
                InsWaypoint = mysql_query(mysql_handler, "INSERT INTO `WPVerbindung` (`Waypoint`, `WPVerbindung`) VALUES ('"..wp.name.."', '"..otherwp.."')")
            end
        end
        fh:close();
    end
...
function gotoPosition(_player, _x, _y, _z, fight)
    ...
    elseif(ainpc.LastPosUpdate + 400 < GetTickCount())then
